...
- Admin：S3/R2 备份 daemon，负责调度、执行、删除收敛与保留清理；
  以及账号冻结恢复 worker（容量熔断的自适应并发下调与到期探测解冻）和账号自动唤醒 worker；
...
自动唤醒由 Admin 解释定时与重置触发，复用账号连接测试；额度事实仍由 Provider 解释。
Store 在 `account_auto_wake` 保存账号计划和执行游标，以配置代次和旧游标原子领取请求；配置写入
保留并发领取产生的消费记录，并与安全审计同事务提交。发送前持久化领取意味着崩溃时可能漏发一次，
但不会以重试不确定请求的方式重复消耗额度。删除账号级联删除计划。
